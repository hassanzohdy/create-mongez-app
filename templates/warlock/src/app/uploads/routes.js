import { deleteFile, getUploadedFile, getUploadedFileUsingHash, router, uploadChunkedFiles, uploadFiles, } from "@mongez/warlock";
import { adminPath, guarded } from "app/utils/router";
guarded(() => {
    // Upload files
    router.post(["/uploads", adminPath("/uploads")], uploadFiles);
    // Upload chunked files
    router.post(["/uploads/chunks", adminPath("/uploads/chunks")], uploadChunkedFiles);
