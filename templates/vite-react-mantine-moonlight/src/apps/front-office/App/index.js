...
const defaultThemeValue = cache.get(themeCacheKey, userPrefersDarkMode() ? "dark" : "light");
export default function App({ children }) {
    const [cacheProvider, setCacheProvider] = useState(cacheValue);
    const [colorScheme, setColorScheme] = useState(defaultThemeValue);
    // if you're using hard reload, remove the following hook
    useEvent(() => routerEvents.onLocaleChanging(localeCode => {
        setCacheProvider(localeCode === "ar" ? cacheRTL : cacheLTR);
    }));
    const direction = currentDirection();
    const primaryFontFamily = getPrimaryFont();
    return (<>
      <ColorSchemeProvider colorScheme={colorScheme} toggleColorScheme={theme => {
            if (!theme)
                return;
        ...
        <CacheProvider value={cacheProvider}>
          <MantineProvider theme={{
            ...
            dir: direction,
        }} emotionCache={isRTL() ? cacheRTL : cacheLTR} withGlobalStyles withNormalizeCSS>
            <InjectThemeAtom />
            <ModalsProvider>
              <ToastContainer />
              {children}
 ...
 */
export function AppWithUser({ children }) {
