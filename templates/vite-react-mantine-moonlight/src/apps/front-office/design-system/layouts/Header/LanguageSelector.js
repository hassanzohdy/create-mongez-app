...
export function LanguageSelector() {
    ...
    const items = data.map(item => (<Menu.Item icon={<Image src={item.image} width={18} height={18}/>} onClick={() => {
            changeLocaleCode(item.localeCode);
            setSelected(item);
        }} key={item.label}>
      {item.label}
    </Menu.Item>));
    return (<Menu onOpen={() => setOpened(true)} onClose={() => setOpened(false)} radius="md" width="target">
      <Menu.Target>
